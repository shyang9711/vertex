...
def sql_has_monthly_state() -> bool:
    if not sql_db_ready():
        return False
    try:
        data = load_monthly_state_payload()
...
def sql_has_any_match_rules() -> bool:
    if not sql_db_ready():
        return False
    try:
        return bool(load_json_files("match_rules"))
    except Exception:
        return False
...
def migrate_missing_json_to_sql() -> dict:
    ...
    try:
        io = _io()
        clients_json = io._read_json_file(io.DATA_FILE, []) if _json_file_present(io.DATA_FILE) else None
        ams_json = (
            io._read_json_file(io.ACCOUNT_MANAGERS_FILE, [])
            if _json_file_present(io.ACCOUNT_MANAGERS_FILE) else None
        )
        tasks_json = io._read_json_file(io.TASKS_FILE, []) if _json_file_present(io.TASKS_FILE) else None
        monthly_json = (
            io._read_json_file(io.MONTHLY_STATE_FILE, {})
            if _json_file_present(io.MONTHLY_STATE_FILE) else None
        )
        ...
        if io.MATCH_RULES_DIR.exists():
            ...
            if sql_db_ready():
                try:
                    existing_names = set(load_json_files("match_rules").keys())
                except Exception:
                    existing_names = set()
        ...
        need_clients = isinstance(clients_json, list) and len(clients_json) > 0 and not sql_has_list("clients")
        need_ams = isinstance(ams_json, list) and len(ams_json) > 0 and not sql_has_list("account_managers")
        need_tasks = isinstance(tasks_json, list) and len(tasks_json) > 0 and not sql_has_list("tasks")
        need_monthly = (
            isinstance(monthly_json, dict) and len(monthly_json) > 0 and not sql_has_monthly_state()
        )
